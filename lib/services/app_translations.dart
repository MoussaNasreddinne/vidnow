import 'package:get/get.dart';

class AppTranslations extends Translations {
  @override
  Map<String, Map<String, String>> get keys => {
        // ENGLISH Translations
        'en': {
          'appName': 'VidNow',
          'loading': 'Loading...',
          'error': 'Error',
          'all': 'All',
          'noFavoritesAdded': 'No favorites added yet. Click the heart icon on a video to add it!',
          'failedToGetVideoStream': 'Failed to get video stream: @error',
          'noVideosFound': 'No videos found for this category.',
          'liveChannels': 'Live Channels',
          'noLiveChannels': 'No live channels available.',
          'liveStreamNotAvailable': 'Live stream URL not available for this channel.',
          'favorites': 'Favorites',
          'addedToFavorites': 'Added "@videoTitle" to favorites!',
          'removedFromFavorites': 'Removed "@videoTitle" from favorites!',
          'profileUserName': 'User Name',
          'profileUserEmail': 'user@example.com',
          'editProfile': 'Edit Profile',
          'settings': 'Settings',
          'watchHistory': 'Watch History',
          'notifications': 'Notifications',
          'helpAndSupport': 'Help & Support',
          'logout': 'Logout',
          'playbackError': 'Playback Error',
          'couldNotPlayVideo': 'Could not play video: @errorMessage',
          'playbackCouldNotBeStarted': 'Video playback could not be started.\nDouble-check the video URL or network.',
          'languageEnglish': 'English',
          'languageArabic': 'العربية',
          'languageFrench': 'Français',
        },
        // ARABIC Translations
        'ar': {
          'appName': 'فيد ناو',
          'loading': 'جاري التحميل...',
          'error': 'خطأ',
          'all': 'الكل',
          'noFavoritesAdded': 'لم تتم إضافة أي مفضلات بعد. انقر على أيقونة القلب على الفيديو لإضافته!',
          'failedToGetVideoStream': 'فشل في الحصول على بث الفيديو: @error',
          'noVideosFound': 'لم يتم العثور على فيديوهات لهذه الفئة.',
          'liveChannels': 'قنوات البث المباشر',
          'noLiveChannels': 'لا توجد قنوات بث مباشر متاحة.',
          'liveStreamNotAvailable': 'رابط البث المباشر غير متوفر لهذه القناة.',
          'favorites': 'المفضلة',
          'addedToFavorites': 'تمت إضافة "@videoTitle" إلى المفضلة!',
          'removedFromFavorites': 'تمت إزالة "@videoTitle" من المفضلة!',
          'profileUserName': 'اسم المستخدم',
          'profileUserEmail': 'user@example.com',
          'editProfile': 'تعديل الملف الشخصي',
          'settings': 'الإعدادات',
          'watchHistory': 'سجل المشاهدة',
          'notifications': 'الإشعارات',
          'helpAndSupport': 'المساعدة والدعم',
          'logout': 'تسجيل الخروج',
          'playbackError': 'خطأ في التشغيل',
          'couldNotPlayVideo': 'تعذر تشغيل الفيديو: @errorMessage',
          'playbackCouldNotBeStarted': 'تعذر بدء تشغيل الفيديو.\nتحقق مرة أخرى من رابط الفيديو أو الشبكة.',
          'languageEnglish': 'English',
          'languageArabic': 'العربية',
          'languageFrench': 'Français',
        },
        // FRENCH Translations
        'fr': {
          'appName': 'VidNow',
          'loading': 'Chargement...',
          'error': 'Erreur',
          'all': 'Tout',
          'noFavoritesAdded': 'Aucun favori ajouté. Cliquez sur le cœur pour en ajouter un !',
          'failedToGetVideoStream': 'Échec du chargement du flux vidéo : @error',
          'noVideosFound': 'Aucune vidéo trouvée pour cette catégorie.',
          'liveChannels': 'Chaînes en direct',
          'noLiveChannels': 'Aucune chaîne en direct disponible.',
          'liveStreamNotAvailable': 'URL du flux en direct non disponible pour cette chaîne.',
          'favorites': 'Favoris',
          'addedToFavorites': '"@videoTitle" a été ajouté aux favoris !',
          'removedFromFavorites': '"@videoTitle" a été retiré des favoris !',
          'profileUserName': 'Nom d\'utilisateur',
          'profileUserEmail': 'user@example.com',
          'editProfile': 'Modifier le profil',
          'settings': 'Paramètres',
          'watchHistory': 'Historique',
          'notifications': 'Notifications',
          'helpAndSupport': 'Aide et support',
          'logout': 'Se déconnecter',
          'playbackError': 'Erreur de lecture',
          'couldNotPlayVideo': 'Impossible de lire la vidéo : @errorMessage',
          'playbackCouldNotBeStarted': 'La lecture vidéo n\'a pas pu démarrer.\nVérifiez l\'URL ou le réseau.',
          'languageEnglish': 'English',
          'languageArabic': 'العربية',
          'languageFrench': 'Français',
        }
      };
}